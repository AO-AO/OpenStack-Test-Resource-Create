...
nova delete `nova list --all-tenant | grep HA| grep rec |awk -F '|' '{print $2}'`

while [[ `nova list --all-tenant | grep HA|grep rec | wc -l` != 0 ]];
do
    echo "Waiting for instances soft-deleting"
done

for instance in `nova list --all-tenant --status=SOFT_DELETED |grep rec| grep HA |awk -F '|' '{print $2}'`
do
  nova force-delete $instance
done

while [[ `nova list --all-tenant --status=SOFT_DELETED |grep rec| grep HA | wc -l` != 0 ]];
do
...
for volume in `cinder list --all-tenant | grep instance| grep "error_deleting" |awk -F '|' '{print $2}'`
do
