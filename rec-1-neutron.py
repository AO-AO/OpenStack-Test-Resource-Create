...
def create_net(project_name, token):
    ...
    params = {"network": {"name": project_name + '-net' + '-rec', "admin_state_up": 'true'}}
...
def create_subnet(project_name, net, token):
    ...
    params = {"subnet": {"network_id": net['id'], "ip_version": 4, "cidr": "192.168.15.0/24", "gateway_ip": "192.168.15.1", "name": project_name + '-subnet' + '-rec'}}
    headers = {'X-Auth-Token': token,
               'Content-Type': 'application/json'}
    data = json.dumps(params)
    res = requests.post(url, data = data, headers = headers)
...
def router_list(project_name, token):
    project_id = None
    for project in user_map:
...
for vdc in tokens.keys():
    ...
    router = router_list(vdc, token)
    output = router_interface_add(router, subnet, token)
    print output
