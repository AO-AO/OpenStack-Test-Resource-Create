...
def create_instance(project_name, user_name, node_name, network, image, flavor, token, type='Init'):
    project_id = None
    for project in user_map:
        if project_name == project.get('project_name'):
            project_id = project['project_id']
    ...
    vol_size = image['vol_size']
    flavor_id = flavor.get('id', "11")
    params = {"server": {"name": instance_name, "imageRef": image['id'], "availability_zone": node_name, "key_name": "nova-key", "block_device_mapping_v2": [{"source_type": "image", "uuid": image['id'], "boot_index": "0", "volume_size": vol_size, "destination_type": "volume", "delete_on_termination": "true"}], "flavorRef": flavor_id, "max_count": 1, "min_count": 1, "networks": [{"uuid": network['id']}]}}
    data = json.dumps(params)
    res = requests.post(url, data = data, headers = headers)
    result = res.json().values()[0]
    return result

def key_gen(project_name, token):
    project_id = None
    for project in user_map:
        if project_name == project.get('project_name'):
            project_id = project['project_id']
            break
    url = "http://%(manage_ip)s:8774/v2/%(project_id)s/os-keypairs" % {'manage_ip': manage_ip, 'project_id': project_id}
    headers = {'X-Auth-Token': token,
               'Content-Type': 'application/json'}
    params = {"keypair": {"public_key": pub_key, "name": "nova-key"}}
    data = json.dumps(params)
    res = requests.post(url, data = data, headers = headers)
    result = res.json()
    return result
...
for vdc in tokens.keys():
    ...
    sel_images = []
    for image in images:
        if (image['name'] == 'CentOS-HA') or (image['name'] == 'Windows-HA'):
            sel_images.append(image)

    flavors = flavor_list(vdc, token)
    sel_flavor = None
    for flavor in flavors:
        if flavor['name'] == '2C2G0G':
            sel_flavor = flavor
            break 
